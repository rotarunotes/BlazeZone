...
**1.** Which two traffic types use the Real-Time Transport Protocol (RTP)? _(Choose two)_
...
**11.** Three bank employees are using the corporate network. The first employee uses a web browser to view a company web page. The second employee accesses the corporate database to perform financial transactions. The third employee participates in a live audio conference. If QoS is implemented, what will be the priorities from highest to lowest?
...
**12.** 
![[Pasted image 20260513113125.png]]___________

---

**13.** 
![[Pasted image 20260513113110.png]]

---

**14.** Match the characteristic to the forwarding method. _(Not all options are used)_
...
Options: _low latency / always stores the entire frame / may forward runt frames / checks the CRC before forwarding / begins forwarding when the destination address is received / checks the frame length before forwarding_

---

**15.** The IP address of which device interface should be used as the default gateway setting of host H1?
...
**17.** Match the network with the correct IP address and prefix that will satisfy the usable host addressing requirements for each network. _(Not all options are used)_

- Network A (100 hosts) → ___________
- Network B (50 hosts) → ___________
- Network C (25 hosts) → ___________
- Network D (2 hosts) → ___________

---

**18.** Which protocol was responsible for building the ARP table shown in the exhibit?
...
**19.** A network administrator notices that some newly installed Ethernet cabling is carrying corrupt and distorted data signals near fluorescent lights and electrical equipment. Which two factors may interfere with the copper cabling? _(Choose two)_
...
**23.** A network administrator is adding a new LAN to a branch office. The new LAN must support 25 connected devices. What is the smallest network mask that the network administrator can use?
...
**26.** A technician with a PC is using multiple applications while connected to the Internet. How is the PC able to keep track of the data flow between multiple application sessions?
...
**27.** A network administrator is adding a new LAN to a branch office. The new LAN must support 61 connected devices. What is the smallest network mask that the network administrator can use?
...
**28.** Match the network with the correct IP address and prefix that will satisfy the usable host addressing requirements for each network. _(Not all options are used)_
...
- A student is using a VoIP phone; the unique identifier burned into the phone is a transport layer address
- A student is playing a web-based movie; the movie and sound are encoded within the transport layer header
- A student has two web browser windows open; the transport layer ensures the correct web page is delivered to the correct browser window
- A corporate worker is accessing a web server; the transport layer formats the screen so the web page appears properly no matter what device is used

---

**33.** Host B on subnet Teachers transmits a packet to host D on subnet Students. Which Layer 2 and Layer 3 addresses are contained in the PDUs transmitted from host B to the router?

- Layer 2 dest = 00-00-0c-94-36-ab / Layer 2 src = 00-00-0c-94-36-bb / Layer 3 dest = 172.16.20.200 / Layer 3 src = 172.16.10.200
- Layer 2 dest = 00-00-0c-94-36-dd / Layer 2 src = 00-00-0c-94-36-bb / Layer 3 dest = 172.16.20.200 / Layer 3 src = 172.16.10.200
- Layer 2 dest = 00-00-0c-94-36-cd / Layer 2 src = 00-00-0c-94-36-bb / Layer 3 dest = 172.16.20.99 / Layer 3 src = 172.16.10.200
- Layer 2 dest = 00-00-0c-94-36-ab / Layer 2 src = 00-00-0c-94-36-bb / Layer 3 dest = 172.16.20.200 / Layer 3 src = 172.16.100.200
...
**35.** An administrator is trying to configure the switch but receives an error message. What is the problem?
...
**36.** Which two protocols operate at the top layer of the TCP/IP protocol suite? _(Choose two)_
...
**37.** A company has a file server with a folder named Public. Read-Only rights are assigned to anyone who can log in, while Edit rights are assigned only to the network admin group. Which component is addressed in the AAA network service framework?
...
**38.** What three requirements are defined by the protocols used in network communications to allow message transmission across a network? _(Choose three)_
...
**39.** What are two characteristics of IP? _(Choose two)_
...
**40.** An employee remotely logs into the company using username and password. A video conference is in progress. After a successful login, the connection to the ISP failed but a secondary connection activated within seconds unnoticed. What three network characteristics are described? _(Choose three)_
...
**41.** What are two common causes of signal degradation when using UTP cabling? _(Choose two)_
...
**43.** On the basis of a tracert output, which two statements about network connectivity are correct? _(Choose two)_
...
**44.** Which two statements describe how to assess traffic flow patterns and network traffic types using a protocol analyzer? _(Choose two)_
...
**46.** Which three layers of the OSI model map to the application layer of the TCP/IP model? _(Choose three)_
...
**47.** If PC1 is sending a packet to PC2 and routing has been configured between the two routers, what will R1 do with the Ethernet frame header attached by PC1?
...
**49.** What are two features of ARP? _(Choose two)_
...
**50.** A network administrator is adding a new LAN to a branch office. The new LAN must support 90 connected devices. What is the smallest network mask that the network administrator can use?
...
**51.** What are two ICMPv6 messages that are not present in ICMP for IPv4? _(Choose two)_
...
**55.** Which two functions are performed at the LLC sublayer of the OSI Data Link Layer to facilitate Ethernet communication? _(Choose two)_
...
**58.** Match the type of threat with the cause. _(Not all options are used)_

- electrical threats → ___________
- hardware threats → ___________
- environmental threats → ___________
- maintenance threats → ___________

---

**59.** A disgruntled employee is using free wireless networking tools to determine information about enterprise wireless networks and plans to use this information to hack the wireless network. What type of attack is this?
...
**62.** What are two problems that can be caused by a large number of ARP request and reply messages? _(Choose two)_
...
**63.** A group of Windows PCs in a new subnet has been added to an Ethernet network. The PCs can access local network resources but not Internet resources. Which three Windows CLI commands and utilities will provide the necessary information to troubleshoot? _(Choose three)_
...
**66.** A network administrator needs to keep the user ID, password, and session contents private when establishing remote CLI connectivity with a switch. Which access method should be chosen?
...
**67.** What are the two most effective ways to defend against malware? _(Choose two)_
...
**70.** A network administrator is adding a new LAN to a branch office. The new LAN must support 4 connected devices. What is the smallest network mask that the network administrator can use?
...
**72.** What two security solutions are most likely to be used only in a corporate environment? _(Choose two)_
...
**76.** An administrator defined a local user account with a secret password on router R1 for use with SSH. Which three additional steps are required to configure R1 to accept only encrypted SSH connections? _(Choose three)_
...
**77.** Which two functions are performed at the MAC sublayer of the OSI Data Link Layer to facilitate Ethernet communication? _(Choose two)_
...
**79.** What are the three parts of an IPv6 global unicast address? _(Choose three)_
...
**80.** A network administrator is designing the layout of a new wireless network. Which three areas of concern should be accounted for when building a wireless network? _(Choose three)_
...
**81.** A new network administrator has been asked to enter a banner message on a Cisco device. What is the fastest way to test whether the banner is properly configured?
...
**89.** A user is attempting to access http://www.cisco.com/ without success. Which two configuration values must be set on the host to allow this access? _(Choose two)_
...
**90.** Which two statements accurately describe an advantage or a disadvantage when deploying NAT for IPv4 in a network? _(Choose two)_
...
**92.** PC1 issues an ARP request because it needs to send a packet to PC2. In this scenario, what will happen next?
...
**96.** Match a statement to the related network model. _(Not all options are used)_

- no dedicated server is required → ___________
- client and server roles are set on a per request basis → ___________
- requires a specific user interface → ___________
- a background service is required → ___________

---

**97.** What are two primary responsibilities of the Ethernet MAC sublayer? _(Choose two)_
...
**98.** What three facts can be determined from the viewable output of the `show ip interface brief` command? _(Choose three)_
...
**99.** Match each type of frame field to its function. _(Not all options are used)_

- addressing → ___________
- error detection → ___________
- type → ___________
- frame start → ___________
...
**101.** Match the firewall function to the type of threat protection it provides to the network. _(Not all options are used)_

- prevents access by port number → ___________
- prevents access based on IP or MAC address → ___________
- prevents unsolicited incoming sessions → ___________
- prevents access to websites → ___________
...
**104.** Which two statements are correct in a comparison of IPv4 and IPv6 packet headers? _(Choose two)_
...
**105.** A network administrator wants to have the same network mask for all networks at a small site with the following requirements:

- IP phones – 22 addresses
- PCs – 20 addresses
- Printers – 2 addresses
- Scanners – 2 addresses

Which single subnet mask would make the most efficient use of the available addresses?
...
**107.** A network administrator is adding a new LAN to a branch office. The new LAN must support 200 connected devices. What is the smallest network mask that the network administrator can use?
...
**108.** What are three commonly followed standards for constructing and installing cabling? _(Choose three)_
...
**109.** What is wrong with the displayed cable termination in the exhibit?
...
**110.** Match the characteristic to the category. _(Not all options are used)_
...
**112.** What are two characteristics shared by TCP and UDP? _(Choose two)_
...
**113.** Which two network addresses can be assigned to the network containing 10 hosts? Your answers should waste the fewest addresses, not reuse addresses that are already assigned, and stay within the 10.18.10.0/24 range of addresses. _(Choose two)_
...
**116.** A network administrator is adding a new LAN to a branch office. The new LAN must support 10 connected devices. What is the smallest network mask that the network administrator can use?
...
**118.** The network administrator has assigned the LAN of LBMISS an address range of 192.168.10.0 with a /29 prefix. The technician decides to use the fifth subnet. By company policies, the router interface is always assigned the first usable host address and the workgroup server is given the last usable host address. Which configuration should be entered into the properties of the workgroup server?
...
**119.** The switches are in their default configuration. Host A needs to communicate with host D, but host A does not have the MAC address for its default gateway. Which network hosts will receive the ARP request sent by host A?
...
**121.** A network engineer has been given the network address of 192.168.99.0 and a subnet mask of 255.255.255.192 to subnet across four networks. How many total host addresses are unused across all four subnets?
...
**126.** Match the header field with the appropriate layer of the OSI model. _(Not all options are used)_
...
**127.** The switches have a default configuration. Host A needs to communicate with host D, but host A does not have the MAC address for the default gateway. Which network devices will receive the ARP request sent by host A?
...
**129.** What two ICMPv6 message types must be permitted through IPv6 access control lists to allow resolution of Layer 3 addresses to Layer 2 MAC addresses? _(Choose two)_
...
**131.** Two pings were issued from a host on a local network. The first ping was issued to the IP address of the default gateway and it failed. The second ping was issued to the IP address of a host outside the local network and it was successful. What is a possible cause for the failed ping?
...
**134.** If host A sends an IP packet to host B, what will the destination address be in the frame when it leaves host A?
...
**136.** Which two statements are correct about MAC and IP addresses during data transmission if NAT is not involved? _(Choose two)_
...
**138.** What are three characteristics of the CSMA/CD process? _(Choose three)_
...
**140.** Which two commands can be used on a Windows host to display the routing table? _(Choose two)_
...
**141.** What are two functions that are provided by the network layer? _(Choose two)_
...
**142.** Which two statements describe features of an IPv4 routing table on a router? _(Choose two)_
...
**145.** Match each description with the corresponding term. _(Not all options are used)_

- the process of placing one message format inside another message format → ___________
- the process of breaking up a long message into individual pieces before being sent over the network → ___________
- the process of converting information from one format into another acceptable for transmission → ___________
...
**148.** Data is being sent from a source PC to a destination server. Which three statements correctly describe the function of TCP or UDP in this situation? _(Choose three)_
...
**149.** Match each description with the corresponding TCP mechanism. _(Not all options are used)_

- number of bytes a destination device can accept and process at one time → ___________
- used to identify missing segments of data → ___________
- method of managing segments of data loss → ___________
- received by a sender before transmitting more segments in a session → ___________

---

**150.** A company uses the address block of 128.107.0.0/16 for its network. What subnet mask would provide the maximum number of equal size subnets while providing enough host addresses for each subnet (largest subnet has 100 hosts)?
...
**152.** Match each item to the type of topology diagram on which it is typically identified. _(Not all options are used)_

- location of a desktop PC in a classroom → ___________
- path of cables that connect rooms to wiring closets → ___________
- IP address of a server → ___________

---

**153.** What two pieces of information are displayed in the output of the `show ip interface brief` command? _(Choose two)_
...
**154.** A user is complaining that an external web page is taking longer than normal to load. The web page does eventually load. Which tool should the technician use with administrator privileges in order to locate where the issue is in the network?
...
**155.** Which value, contained in an IPv4 header field, is decremented by each router that receives a packet?
...
**156.** A network technician is researching the use of fiber optic cabling in a new technology center. Which two issues should be considered before implementing fiber optic media? _(Choose two)_
...
**157.** Match each description with an appropriate IP address. _(Not all options are used)_

- an experimental address → ___________
- a link-local address → ___________
- a public address → ___________
- a loopback address → ___________
